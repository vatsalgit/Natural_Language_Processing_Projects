...
def read_file(file_name):
    lines=None
    with codecs.open(file_name,'r',encoding='utf8') as file_data:
        lines = [line.strip() for line in file_data]
    return lines
 
def tag_seperator(word_and_tag):
        tag = word_and_tag[len(word_and_tag)-2:]
        word = word_and_tag[:len(word_and_tag)-2-1]
        return (word,tag)    

def process_lines(lines):
    list_of_word_tag_sentences = []
    for each_line in lines:
        words = each_line.split(" ")
        list_of_words = []
        for each_word in words:
            list_of_words.append(tag_seperator(each_word))
        list_of_word_tag_sentences.append(list_of_words)
    return list_of_word_tag_sentences


def check_accuracy(file_name_a, file_name_b):
    lines_a = read_file(file_name_a)
    lines_b = read_file(file_name_b)
    a_data = process_lines(lines_a)
    b_data = process_lines(lines_b)
    count_correct = 0
    count = 0.0
...
file_name_a = 'hw5-data-corpus/catalan_corpus_dev_tagged.txt'
file_name_b = 'hmmoutput_new.txt'
print ("accuracy ",check_accuracy(file_name_a, file_name_b)) 
